...
| `ev4-ce-to-builder-transition@1.0.0` | `rezahh107/EV4-Project-Gate` | `project-gate-orchestration` | High: must orchestrate official CE/Builder tools without duplicating specialist logic | No | Orchestration baseline implemented; public CLI exposure not implemented; owner-fixture integration verified; real non-synthetic handoff remains insufficient evidence. |
...
| Builder→Responsive input package | `rezahh107/EV4-Responsive-Architect` | `documented-not-implemented` | High: inventing it in Project Gate would duplicate Responsive authority | No | Responsive boundary is documented, but Project Gate transition is not implemented. |
