...
| Optional EV4-PCVP v1 carrier reader | Decision Kernel canonical contract; Project Gate boundary adapter | Exact non-authoritative schema bytes pinned from `EV4-Decision-Kernel` | Legacy-compatible intake, malformed/unsupported rejection, mechanical reference/Authorization/projection checks, lossless intake-result surface |
...
Active lock files under `contracts/locks/` pin exact repositories, commits, paths and relevant file-byte SHA-256 values. The current B2R lock does not include an official Builder viewport emitter because no such emitter exists at the pinned Builder commit. Historical and immutable owner contracts are unchanged by this documentation repair.

`contracts/locks/pcvp-v1.lock.json` pins the four canonical carrier schemas to
an immutable Decision Kernel commit. The local schema bytes are explicitly
non-authoritative. Producer emission, specialist-output propagation, adoption,
and strict activation remain disabled.
