...
- When `kernel_repo_path` is supplied for producer mode, authoritative Preflight forwards that exact path to owner-backed inspection, request identity includes it, and runtime reuses it. Path substitution after Preflight invalidates the fingerprint before dispatch.
