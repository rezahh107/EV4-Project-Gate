...
def verify_owner_authority(
    ...
    return OwnerAuthority(root, root / VALIDATOR_PATH, root / BUNDLE_ROOT, root / binding["path"], binding), []
...
def run_owner_bridge(authority: OwnerAuthority, carrier_document: dict[str, Any]) -> tuple[dict[str, Any] | None, list[Diagnostic]]:
    ...
    request = {
        ...
        "validator_path": str(authority.validator_path),
        "bundle_root": str(authority.bundle_root),
...
def profile_integration_diagnostics(
    ...
    *,
    ...
    preauth = profile.get("preauthorized_effects")
    preauth_classes = {item.get("effect_class") for item in preauth if isinstance(item, dict) and isinstance(item.get("effect_class"), str)} if isinstance(preauth, list) else set()
    ...
    for index, auth in enumerate(auths):
        ...
        for effect in linked:
            ...
            if auth.get("basis") == "PROFILE_PREAUTHORIZED" and effect_class not in preauth_classes:
                diagnostics.append(diagnostic("PG_PCVP_PROFILE_PREAUTHORIZATION_REJECTED", "error", "The source Profile does not explicitly preauthorize this Effect class.", path, authorization_id=auth_id, effect_id=effect.get("effect_id"), effect_class=effect_class, validator_layer="PROFILE_INTEGRATION"))
