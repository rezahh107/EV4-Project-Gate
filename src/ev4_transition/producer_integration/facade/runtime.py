...
def execute_producer_handoff(
    ...
    *,
    ...
    """Execute one producer-emitted lifecycle from current boundary evidence."""
...
def required_repository_fields(resolved_transition: str) -> tuple[str, ...]:
    return _legacy.required_repository_fields(resolved_transition)
...
def _capture_and_inspect(
    ...
    if diagnostic is not None:
        return None, _legacy._empty_failure(
            "invalid",
            "operator_path_validation_failed",
            diagnostic,
        )
    ...
    result["diagnostics"] = sorted(
        ...
        key=_legacy._diagnostic_key,
    )
    return snapshot, result


def _project_gate_root(
    value: str | Path,
