...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "transition":
        try:
            bundle = load_json_file(Path(args.bundle))
...
def _run_transition(args: argparse.Namespace, bundle: dict[str, Any]) -> dict[str, Any]:
    if args.transition_name == "architect-to-ce":
        hooks = TransitionValidatorHooks(architect=lambda payload: run_architect_validator(args.architect_repo, payload), ce=lambda payload, source_bundle: run_ce_validator(args.ce_repo, payload, source_bundle))
        return transition_from_local_paths(bundle, args.schema_root, args.lock or "contracts/locks/architect-to-ce-transition.v1.lock.json", args.architect_repo, args.ce_repo, validator_hooks=hooks)
...
def _coverage_command(args: argparse.Namespace) -> int:
    try:
        ...
        payload = {"schema_version": "behavioral-coverage-report.v1", "source": getattr(args, "source", None), "status": "source_unparseable", "rule_count": 0, "diagnostics": [{"code": "PG_BRC_SOURCE_UNPARSEABLE", "severity": "error", "message": str(exc), "path": "$"}]}
